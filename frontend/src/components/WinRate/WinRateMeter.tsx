...
export function WinRateMeter({ value, prev, components, trend = 'stable' }: WinRateMeterProps) {
  ...
  return (
    <div className="win-rate-meter rounded-xl border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/5 p-4 space-y-3">
      {/* Header */}
      ...
      <div className="space-y-1">
        <div className="flex items-end justify-between">
          ...
          {prev !== undefined && (
            <span className="text-xs text-slate-500 dark:text-white/30">이전: {prev}%</span>
          )}
        </div>
          ...
          {(Object.keys(WEIGHTS) as Array<keyof WinRateComponents>).map(k => {
            ...
            return (
              <div key={k} className="flex items-center gap-2">
                <span className="text-[10px] text-slate-500 dark:text-white/35 w-24 shrink-0">{COMPONENT_LABELS[k]}</span>
                <div className="flex-1 h-1.5 rounded-full bg-slate-200 dark:bg-white/8">
                  <div
                    className="h-full rounded-full bg-cyan-500/80 dark:bg-cyan-400/60 transition-all duration-500"
                    style={{ width: `${v * 100}%` }}
                  />
                </div>
                <span className="text-[10px] text-slate-500 dark:text-white/30 w-8 text-right">{contribution}%</span>
              </div>
            );
          })}
        </div>
